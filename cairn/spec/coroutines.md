...
`SaslYield` SHALL carry `Respond(Vec<u8>)` for raw, not yet base64-encoded bytes to send, and `AwaitChallenge` when the mechanism has nothing to send and needs the peer's next challenge. An empty `Respond` payload is an acknowledgement, which a protocol whose framing already ended the exchange MAY drop instead of writing.

### Requirement: Initial response
A mechanism answering `Start` with `Respond` has an initial response (RFC 4422); one answering `AwaitChallenge` is server-first. Whether the protocol carries the initial response inline, and whether it should against a given server, SHALL NOT be decided here.
