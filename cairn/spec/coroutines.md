...
`SaslArg` SHALL carry three cases: `Start` before anything is exchanged, `Challenge(&[u8])` for an already base64-decoded peer challenge, and `PeerFinished` when the peer ended the exchange without a further challenge. The first resume of an exchange SHALL take `Start`.
